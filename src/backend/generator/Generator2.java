...
//	This adds the POS part as well
public class Generator2{
	...
	public static String generateStatement(int charLimit, String givenWord, String inputDir){
		...
		continueGeneration(nGrams, posList, charLimit, statement, tagger);
	...
	private static boolean continueGeneration(List<NGram> nGrams, List<POSEntity> posList, int charLimit, List<String> statement, MaxentTagger tagger){
...
//				System.out.println("Start phrase is:\t"+ phrase);
				...
				for (NGram nGram : nGrams)
					if (nGram.getPhrase().contains(phrase)&& nGram.getWords()[nGram.getWords().length- 1].equals(statement.get(i))&& !nGram.getPhrase().equals(phrase)) {
						...
						//	Here we add the POS part
						String expectedSentec= nGram.getPhrase();
						for(int t= i+ 1; t< statement.size(); t++) {
							expectedSentec+= " "+ statement.get(t);
						}
						String[] taggedString= tagger.tagString(expectedSentec).split(" ");
						int posInstances= 0;
						loop:
						for(POSEntity posEntity: posList) {
							if(posEntity.getN()< nGram.getN()&& posEntity.getN()>= statement.size())	continue;
...
//						Here we add the POS part
						String expectedSentec= "";
						for(int t= 0; t< statement.size(); t++) {
							expectedSentec+= statement.get(t)+ " ";
						}
						String[] nGramWords= nGram.getWords();
						for(int t= 0; t< nGramWords.length; t++) {
							if(!expectedSentec.contains(nGramWords[t]))
								expectedSentec+= nGramWords[t]+ " ";
						}
						String[] taggedString= tagger.tagString(expectedSentec.trim()).split(" ");
						int posInstances= 0;
						loop:
						for(POSEntity posEntity: posList) {
							if(posEntity.getN()< nGram.getN()&& posEntity.getN()>= statement.size())	continue;
							...
							for(int n= 1; n<= posEntity.getN(); n++) {
								...
								if(!taggedString[taggedString.length- n].split("_")[1].equals(posEntity.getWords()[posEntity.getWords().length- n]))	continue loop;
									...
									System.out.println(expectedSentec);
									System.out.println(Arrays.toString(taggedString));
									System.out.println(taggedString.length+ "-");
									System.out.println(posEntity.getWords().length);
		...
		return continueGeneration(nGrams, posList, charLimit, statement, tagger);
	}
